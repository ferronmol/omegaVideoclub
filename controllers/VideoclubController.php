    ...
    public function guardarModificacionPelicula($idPelicula)
    {
        ...
        if ($_SESSION['usuario']->getRol() == 1) {
            ...
            if ($exito) {
                ...
                //ternario para que si no hay usuario logueado ponga anonimo
                $username = isset($_SESSION['usuario']) ? $_SESSION['usuario']->getUsername() : 'Anónimo';
                $rol = isset($_SESSION['usuario']) ? $_SESSION['usuario']->getRol() : 'Anónimo';
                $logController->logAdminAction($username, $rol, $accion = 'Modificación de la película  ' . $titulo);
    ...
    public function crearPelicula()
    {
        ...
        if ($_SESSION['usuario']->getRol() == 1) {
            ...
            if ($newexito) {
                ...
                $username = isset($_SESSION['usuario']) ? $_SESSION['usuario']->getUsername() : 'Anónimo';
                $rol = isset($_SESSION['usuario']) ? $_SESSION['usuario']->getRol() : 'Anónimo';
                $logController->logAdminAction($username, $rol, $accion = 'Creación de nueva película  ' . $titulo);
                //muestro un mensaje de exito con la modificacion
    ...
    public function borrarPelicula($idPelicula)
    {
        ...
        if ($_SESSION['usuario']->getRol() == 1) {
            ...
            if ($exito) {
                ...
                $username = isset($_SESSION['usuario']) ? $_SESSION['usuario']->getUsername() : 'Anónimo';
                $rol = isset($_SESSION['usuario']) ? $_SESSION['usuario']->getRol() : 'Anónimo';
                $logController->logAdminAction($username, $rol, $accion = 'Borrado de la pelicula ' . $idPelicula);
                //muestro un mensaje de exito con la modificacion
